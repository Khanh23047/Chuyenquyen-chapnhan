...
def nghingoi(delay):
	for x in range(delay,0,-1):
		print("\r\033[1;93m  🌸 \033[1;91m ~>       \033[1;92m 🌸      \033[1;91m | "+str(x)+" | \r ", end='')
		sleep(0.14)
		print("\r\033[1;91m  🌸 \033[1;91m   ~>     \033[1;92m 🌸     \033[0;31m | "+str(x)+" | \r ", end='')
		sleep(0.14)
		print("\r\033[1;92m  🌸 \033[1;91m     ~>   \033[1;92m 🌸     \033[0;31m | "+str(x)+" | \r", end='')
		sleep(0.14)
		print("\r\033[1;94m  🌸 \033[1;91m       ~> \033[1;92m 🌸   \033[0;31m | "+str(x)+" | \r", end='')

		sleep(0.14)
	print("\r\r\033[1;95m    🌸                         \r", end='')
	sleep(0.1)
def bongoc(so):
	a= "\033[1;31m"+"────"*so
...
class Facebook:
	...
	def get_thongtin(self):
		try:
			...
			ten = get_token["name"]
			self.user_id = get_token['id']
			print(f'{luc}Tên Facebook: {vang}{ten} {red}| {luc}ID: {vang}{self.user_id} ', end='')
			sys.stdout.flush()
			return self.user_id
		except:
			print(red+'Không Get Được Thông Tin, Vui Lòng Thử Lại ! ')
			return 0
	def get_pro5(self):
		headers={
	...
	def chap_nhan(self, id_moi, id_page, name):
		...
		try:
			...
			print(f'{tim} =>>>> {lam}Chấp Nhận Quyền Quản Trị Viên Thành Công')
		except:
			print(red+'Chấp Nhận Quyền Quản Trị Viên Thất Bại ')
	def set_admin(self, id, mk, id_page, name):
		...
		try:
			...
			if str(check) == 'True' or str(check) == 'true':
				...
				print(red+'Mật Khẩu Sai, Vui Lòng Thử Lại !!')
				return 0
		except:
			print(red+'Xác Minh Mật Khẩu Thất Bại, Acc Có Thể Bị Block Hãy Thử Lại Sau !!')
			return 0
		headers={
			'Host':'www.facebook.com',
			'sec-ch-ua':'"Chromium";v="107", "Not=A?Brand";v="24"',
...
while True:
	while True:
		cookie=input (thanh_xau + luc+' Nhập Cookie Nick Cầm Pro5: '+vang)
		mk = input (thanh_xau + luc+' Nhập Mật Khẩu Nick Cầm Pro5 : '+den)
		fb = Facebook(cookie)
		bongoc(14)
		a=fb.get_thongtin()
		if a == 0:
	...
	if chon == 'y':
		...
		id = input (thanh_xau+luc+'Nhập ID Nick Cần Chuyển Admin: '+vang)
	dl = int(input(thanh_xau+luc+' Vui Lòng Nhập Delay : '+vang))
	for x in data_pro5:
